...
char oled_text[3][17];
void sdd1306_log(uint8_t row,char* text){
	snprintf(oled_text[row],17, "%s", text);
}
void ssd1306_task(void* arg){
	...
	i2c_master_inito(&dev, CONFIG_SDA_GPIO, CONFIG_SCL_GPIO, CONFIG_RESET_GPIO);
	...
	ssd1306_display_text(&dev, 0, "MPU9250 Data:", 13, false);
	ssd1306_display_text(&dev, 1, "Hello World!!", 13, false);
	ssd1306_display_text(&dev, 2, "NEO-6M Data:", 12, false);
	ssd1306_display_text(&dev, 3, "Hello World!!", 13, false);

	while(1){
		ssd1306_display_text(&dev, 1, oled_text[0], 16, false);
		ssd1306_display_text(&dev, 3, oled_text[1], 16, false);
