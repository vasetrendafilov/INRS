...
void mpu6050_complimentary_filter(void* arg)
{
    ...
    while (1) {
        ...
        #if 0   //Make 1 to print raw gyro values
        printf("MPU6050 raw gyro values...\n");
        if (ret == ESP_OK) {
            disp_buf(gyro_raw_value, BUFF_SIZE/2);
        }
        #endif
        ...
        xSemaphoreTake(print_mux, portMAX_DELAY);
        #if 0   //Make 1 to print raw acce values
        //printf("MPU6050 raw acce values...\n");
        if (ret == ESP_OK) {
            disp_buf(acce_raw_value, BUFF_SIZE/2);
        }
        #endif
        xSemaphoreGive(print_mux);
        /*Call complimentary filter*/
        complimentory_filter(acce_raw_value, gyro_raw_value, complimentary_angle, BUFF_SIZE/2);
        xSemaphoreTake(print_mux, portMAX_DELAY);

        #if 0   //Make 1 to print comp gyro values
        printf("%f ", complimentary_angle[0]);
        printf("%f ", complimentary_angle[1]);
        #endif
        xSemaphoreGive(print_mux);
...
static void printTask(void* arg)
{
    ...
    while (true) {
        printf("Pitch: %+6.1f \t Roll: %+6.1f \n", complimentary_angle[0], complimentary_angle[1]);
...
void app_main()
{
    ...
    i2c_master_init();
    xTaskCreate(mpu6050_complimentary_filter, "i2c_mpu6050_complimentary_filter", 1024 * 2, NULL, 10, NULL);
    xTaskCreate(printTask, "printTask", 2 * 1024, NULL, 5, NULL);
}
